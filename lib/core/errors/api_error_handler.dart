...
class ApiErrorHandler {
    ...
    try {
      switch (e.response?.statusCode) {
        case 400: // Bad request - validation errors
          AppLogger.logInfo('400 Bad Request: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 400);

        case 401: // Unauthorized
          AppLogger.logInfo('401 Unauthorized: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 401);

        case 403: // Forbidden
          AppLogger.logInfo('403 Forbidden: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 403);

        case 404: // Not found
          AppLogger.logInfo('404 Not Found: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 404);

        case 409: // Conflict
          AppLogger.logInfo('409 Conflict: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 409);

        case 422: // Unprocessable entity
          AppLogger.logInfo('422 Unprocessable Entity: ${e.response!.data.toString()}');
          return _parseErrorResponse(e.response!.data, 422);

        case 500: // Internal server error
          AppLogger.logInfo('500 Internal Server Error');
        ...
        default: // Something went wrong
          AppLogger.logInfo('Unknown error: Status ${e.response?.statusCode}');
      ...
      AppLogger.logInfo('Error parsing response: $parseError');
      return ApiErrorModel(
  ...
  static ApiErrorModel _parseErrorResponse(dynamic data, int? statusCode) {
    try {
      ...
      AppLogger.logInfo('Failed to parse error response: $e');
      return ApiErrorModel(
        message: "Error processing server response",
        icon: Icons.error,
        statusCode: statusCode,
