...
class HomeRepo {
  ...
  getPopularMovies() async {
    try {
      ...
      await homeLocalDataSource.homeLocalDataSourceInit();
      final localResponse = await homeLocalDataSource.getPopularMovies();

      if (localResponse != null && PaginationConstants.pageNumber <= localResponse.page) {
        PaginationConstants.pageNumber = localResponse.page;
        AppLogger.logInfo(
          'Popular movies from local: ${localResponse.results.length}',
        );
      ...
      AppLogger.logInfo(
        'Popular movies from remote: ${response.results.length}',
      );
      await homeLocalDataSource.savePopularMovies(response);
